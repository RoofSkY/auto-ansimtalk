...
flowchart LR
    ...
    REFRESH[공통 갱신 루프] --> STATUS[안심톡 출결 조회]
...
5. 별도 스레드에서 실제 작업을 수행하고, 성공·실패 모두 종료 시 잠금을 해제한다.
...
이 잠금은 **실행 중 중복 요청을 막는 기능**이다. 완료 후 같은 버튼을 다시 누르는 행위를
...
하나의 루프가 출결 조회 → 차량 조회 순서로 작업한다. 자동 조회는 각 설정 스위치를 따르며,
기본 갱신 대기 시간은 60초다. 전체 조회 완료 후 다음 대기 시간을 계산하므로 실제 조회 시작 간격에는
외부 서비스 응답 시간도 포함된다.

| 발생 조건 | 조회 범위 | 카운트다운 |
| --- | --- | --- |
| 주기 도래 | 설정에서 켠 출결·차량 조회 | 전체 주기 갱신 후 재설정 |
| 갱신 버튼 | 출결 + 차량 | 전체 갱신 후 재설정 |
| 출결 필터 선택 | 출결 | 기존 주기 유지 |
| 등하원 작업 종료 | 출결 | 기존 주기 유지 |
| 데이터 복원 완료 | 출결 + 차량 | 전체 갱신 후 재설정 |

수동 갱신은 자동 조회 스위치가 꺼져 있어도 실행한다. 갱신 중 추가 요청은 깨우기 이벤트와
작업 범위 집합에 모아 처리한다. 부분 갱신 중 기존 주기가 지났으면 다음 반복에서 주기 조회를 수행한다.
...
앱이 꺼져 있던 시각의 예약을 나중에 보충 실행하는 기능은 없다.
...
| 트레이 → 입출차 알림 테스트 | 저장된 알림 받기·알림 방식 설정으로 테스트 입차 알림 처리 |
...
로그는 `time`, `type`, `target`, `message`, `ok` 필드를 가진 JSONL로 저장한다.
...
| `refresh_interval` | `60` | 공통 갱신 대기 시간, 최소 10초 |
...
| 항목 | 현재 범위 |
| --- | --- |
| 예약 | 앱 실행 중 현재 시각 일치 검사, 누락 예약 보충·실패 자동 재시도 없음 |
| 중복 방지 | 실행 중 대상 잠금, 예약별 당일 실행 기록. 완료 후 수동 재등록은 가능 |
