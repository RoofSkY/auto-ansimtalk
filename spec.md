...
4. 여러 매를 요청하면 차량 상세에서 권종별 등록 가능 수량과 잔여 수량을 먼저 확인한다.
   요청 매수와 확인된 상한 중 작은 수량을 권종별 한 번의 일괄 요청으로 적용한다.
   가능 수량이 0인 권종은 생략하고, 선택된 다른 권종은 계속 처리한다.
5. 일괄 적용이 거절되어도 1매씩 다시 등록하지 않는다. 응답 유실이나 서버 오류로 결과가 불확실하면
   이후 등록을 중단하고 실제 등록 내역 확인을 안내한다. 명확한 거절은 선택된 다른 권종 처리를 막지 않는다.
6. 차량·권종별 결과를 기록한다. 일부만 적용되면 `성공 매수/요청 매수`를 표시하고 실패로 분류한다.
7. 성공·실패·부분 성공과 관계없이 작업 종료 후 처리 대상 끝번호만 조회해 실제 등록 수량을 다시 확인한다.
...
### 4.4 차량번호 직접 입력으로 주차권 등록·취소

- 모든 페이지 우하단의 `iParking` 버튼으로 직접 등록 창을 연다. 입소자 목록에 차량을 추가하지 않아도 된다.
- 무료·유료권의 보유 잔여 수량을 조회하며 실패하거나 수량을 알 수 없으면 0장으로 표시하지 않는다.
- 뒤 4자리 또는 전체 차량번호로 입차 차량을 검색한다. 뒤 4자리가 같은 차량이 여러 대이면 전체 번호를 선택한다.
- 선택한 차량의 전체 번호, 입차 일시, 총 주차시간, 이미 등록된 주차권을 표시한다. 다른 스토어 등록 내역도 포함한다.
- 권종별 추가 수량은 0부터 선택한다. 상한은 앱 권종 한도·보유 잔여 수량·해당 차량의 등록 가능 수량 중 최솟값이다.
- 수량 입력은 가운데 정렬하며 직접 입력·증가 버튼 모두 100장과 실제 등록 가능 상한을 넘지 않도록 제한한다.
  등록 창의 일반 텍스트는 드래그 선택을 막고 입력칸의 편집은 유지한다.
- 조회 토큰은 등록 접수에 10분간 유효하다. 서버는 차량번호와 입차 이력을 보관하며 브라우저의 임의 차량 정보로 등록하지 않는다.
- 접수된 작업은 기존 차량 작업 큐와 뒤 4자리 잠금을 공유한다. 입소자 버튼·예약과 같은 차량의 동시 등록을 차단한다.
- 작업 시작 시 입차 이력과 수량을 다시 확인한다. 출차·재입차 시 전체 등록을 중단한다.
  특정 권종의 잔여 수량이나 등록 한도가 부족하면 해당 권종만 실패로 기록하고 선택한 다른 권종은 계속 처리한다.
- 직접 등록은 권종별 한 번만 요청한다. 서버가 등록을 거절해도 선택한 다른 권종은 계속 처리한다.
  응답 유실로 등록 여부를 알 수 없으면 이후 등록을 중단한다. 자동 재등록하지 않으며 성공·실패·부분 성공·결과 미확인을 구분한다.
- 동일 조회 토큰의 중복 요청은 기존 결과를 반환한다. 브라우저는 접수 토큰을 탭 세션에 보관하고 결과 조회로 연결 단절을 복구한다.
  서버 재시작으로 토큰이 사라지면 처리 기록과 실제 등록 내역 확인을 안내하며 자동으로 다시 등록하지 않는다.
- 완료 후 등록 내역과 보유 수량을 다시 조회하고 선택 수량은 0으로 초기화한다. 결과에는 전체 차량번호를 기록한다.
- 상세 응답의 무료·유료권 ID가 보유 목록의 권종 ID와 일치하고 잔여 수량이 모두 확인되면
  보유 수량을 재사용하여 별도 재고 조회를 생략한다. 누락·불일치·수량 미확인은 보유 목록을 다시 조회한다.
  등록 가능 수량과 보유 수량은 별도로 취급하고, 오래된 재고 응답이 최신 상세 수량을 덮어쓰지 못하게 한다.
- 처리 중에도 창을 닫을 수 있다. 주차권 처리 중에는 아이파킹 계정 변경을 차단하며 계정 변경·데이터 복원 시 기존 조회 토큰을 무효화한다.
- 등록된 주차권 옆의 `취소` 버튼에서 전체 차량번호·권종을 확인하고 취소 수량을 선택한다.
  현재 스토어에서 등록한 미정산 주차권만 취소할 수 있으며 다른 스토어 등록분은 조회만 가능하다.
  수량은 등록된 수량과 100장 중 작은 값까지 선택하며, 실행 직전 같은 입차 건과 등록 수량을 다시 확인한다.
- 취소는 적용 내역의 정확한 주차권 ID로 아이파킹의 취소 가능 여부를 검증한 뒤 실행한다.
  등록과 같은 차량 작업 잠금·조회 토큰을 사용하며 동일 요청을 재전송해도 중복 취소하지 않는다.
  응답 유실 시 자동 재취소하지 않는다. 결과를 처리 기록의 아이파킹/주차에 남기고 등록 내역·보유 수량·뱃지를 갱신한다.

`src/manual_parking.py`가 조회 토큰과 등록 작업을 관리하고,
`templates/parking.html`, `static/parking.js`, `static/parking.css`가 직접 등록 화면을 구성한다.

| API | 역할 |
| --- | --- |
| `GET /api/parking/inventory` | 무료·유료권 보유 수량 조회 |
| `GET /api/parking/search?number=...` | 입차 차량 후보 조회 |
| `GET /api/parking/detail?plate=...&history=...` | 차량 상세·등록 가능 수량과 조회 토큰 발급 |
| `POST /api/parking/register` | 조회 토큰과 권종별 수량으로 등록 접수 |
| `POST /api/parking/cancel` | 조회 토큰과 적용 주차권 키·수량으로 취소 접수 |
| `GET /api/parking/requests/{token}` | 접수된 작업의 결과 확인 |
...
| 아이파킹 | 저장된 토큰 사용 또는 로그인. 인증 오류 시 세션 갱신 후 1회 재시도. 등록·취소의 비정상 HTML·서버 오류는 결과 미확인으로 중단 |
...
주차권 등록·취소 후에는 처리한 끝번호만 갱신한다. 여러 부분 갱신 요청은 끝번호 집합으로 합치며,
대기 중인 전체 갱신은 부분 요청을 포함한다. 실행 중 들어온 요청은 완료 후 다시 조회한다.
부분 갱신 중 정기 전체 갱신 시각이 되면 전체 조회를 대기열에 넣어 부분 조회 뒤에 실행한다.
전체·부분 차량 조회는 같은 작업자와 잠금을 사용하고, 부분 결과는 조회하지 않은 차량 상태를 유지한다.
입소자 목록에 없는 끝번호는 목록 조회를 생략한다. 정기·수동 전체 갱신과 목록 변경 시 전체 조회는 유지한다.
...
입차 조회 완료 시 행 배경과 입출차 알림을 먼저 반영한다. 이후 주차권 상세 내역을 최대 8개씩
병렬 조회하며, 같은 입차 이력은 한 번만 조회한다. 입차 이력이 바뀐 차량의 수량은 확인 전까지 숨긴다.
동일 입차 이력의 직전 수량은 상세 조회 동안 유지하되, 등록·취소 중에는 숨긴다.
상세 내역 조회 실패는 입출차 판정을 바꾸지 않으며 주차권 수량만 미확인 상태로 처리한다.
부분 조회 성공으로 다른 차량의 조회 오류를 지우지 않으며, 계정·입소자 목록 변경 전 응답은 반영하지 않는다.
